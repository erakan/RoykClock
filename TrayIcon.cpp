...
CTrayIcon::~CTrayIcon(){
	RemoveIcon(); // remove icon from system tray
}
...
//
BOOL CTrayIcon::SetIcon(UINT uID, BOOL bLoadFromRes)
{
    HICON hicon=NULL;
	if (uID) {
		if (!bLoadFromRes)
		{
			CString WindowText;
			AfxGetApp()->m_pMainWnd->GetWindowText(WindowText);
			strncpy(m_nid.szTip, (LPCTSTR) WindowText, sizeof(m_nid.szTip));
		}
		else
		{
			AfxLoadString(uID, m_nid.szTip, sizeof(m_nid.szTip));
...
BOOL CTrayIcon::SetIcon(UINT nID, CString lpTip)
{
	HICON hicon;
	strncpy(m_nid.szTip, (LPCTSTR) lpTip, sizeof(m_nid.szTip));
